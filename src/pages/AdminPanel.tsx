...
const nav = [
  ...
  ['Inventory', '/admin/products', BarChart3],
...
function Products() {
  ...
  return <Shell><header className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow">Catalogue</p><h1 className="mt-4 font-display text-5xl">Products</h1></div><button onClick={() => navigate('/admin/products/new')} className="flex items-center justify-center gap-2 bg-gold px-5 py-3 text-[10px] font-semibold tracking-[0.15em] text-obsidian"><Plus size={15} /> New product</button></header><div className="mb-6 flex flex-col gap-3 sm:flex-row"><div className="flex flex-1 items-center border border-ivory/10 bg-elevated px-4"><Search size={16} className="text-muted" /><input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && load()} placeholder="Search products..." className="w-full bg-transparent px-3 py-3 text-sm text-ivory outline-none" /></div><select value={status} onChange={e => setStatus(e.target.value)} className="border border-ivory/10 bg-elevated px-4 py-3 text-sm text-ivory"><option value="">All statuses</option>{['draft', 'listed', 'verified', 'featured', 'archived'].map(s => <option key={s}>{s}</option>)}</select></div><div className="overflow-x-auto border border-ivory/10 bg-elevated"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-ivory/10 text-[10px] uppercase tracking-[0.15em] text-muted"><tr><th className="p-5">Product</th><th>Price</th><th>Stock</th><th>Status</th><th className="p-5 text-right">Actions</th></tr></thead><tbody>{products.map(product => <tr key={product.id} className="border-b border-ivory/10 last:border-0"><td className="p-5"><div className="flex items-center gap-4">{product.image ? <img src={product.image} className="h-12 w-12 object-cover" alt="" /> : <div className="h-12 w-12 bg-secondary" />}<div><p>{product.name}</p><p className="mt-1 text-xs text-muted">{product.artisan || 'Unassigned'}</p></div></div></td><td>₹{product.price.toLocaleString()}</td><td className={product.stock < 3 ? 'text-gold' : 'text-muted'}>{product.stock}</td><td><span className="border border-gold/30 px-2 py-1 text-[10px] uppercase tracking-wider text-gold">{product.status}</span></td><td className="p-5 text-right"><button onClick={() => navigate(`/admin/products/${product.id}/edit`)} className="mr-4 text-xs text-gold">Edit</button><button onClick={async () => { if (confirm('Archive this product?')) { await adminApi.archiveProduct(product.id); load() } }} className="text-xs text-muted hover:text-red-300"><Archive size={15} /></button></td></tr>)}</tbody></table>{products.length === 0 && <p className="p-12 text-center text-sm text-muted">No products found.</p>}</div></Shell>
}
